...
# Domain Adaptation with addtional pretraining task: Masked Language Model

logger = logging.getLogger(__name__)

# Japanese Morphological Analizer for Bert Tokenizer Input
    ...
    def __init__(self, bert_tokenizer: BertTokenizer, jp_tokenizer:JumanTokenizer, args, file_path='train', block_size=512):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            for _, line in enumerate(docs):
                ...
                tokenized_text = bert_tokenizer.convert_tokens_to_ids(bert_tokenizer.tokenize(" ".join(jp_tokenizer.tokenize(text))))

                # add special tokkens : [CLS] and [SEP]
                added_special =   bert_tokenizer.build_inputs_with_special_tokens(tokenized_text)
...
def my_collate_fn(batch):
    # datasetの出力が
    # seq_ids = dataset[batch_idx]
    # の場合.
    seq_ids = []
    for sample in batch:
        seq_id = sample
        seq_ids.append(seq_id)
    datum = torch.stack(seq_ids, dim=0)
    return datum
...
# マスキング
def mask_tokens(inputs, bert_tokenizer: BertTokenizer, jp_tokenizer: JumanTokenizer, args):
    ...
    labels[(masked_indices != 1.0) ] = -1  # We only compute loss on masked tokens

    # 80% of the time, we replace masked input tokens with tokenizer.mask_token ([MASK])
    indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8))
    inputs[(indices_replaced == 1.0) & (labels == -1)] = bert_tokenizer.convert_tokens_to_ids(bert_tokenizer.mask_token)

    # 10% of the time, we replace masked input tokens with random word
    indices_random = torch.bernoulli(torch.full(labels.shape, 0.5))
    random_words = torch.randint(len(bert_tokenizer), labels.shape, dtype=torch.long)
    inputs[(indices_random == 1.0) & (indices_replaced == 1.0) & (labels == -1)] = random_words[(indices_random == 1.0) & (indices_replaced == 1.0) & (labels == -1)]
...
def train(args, train_dataset, model, bert_tokenizer: BertTokenizer, jp_tokenizer: JumanTokenizer):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, collate_fn=my_collate_fn)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            inputs, labels = mask_tokens(batch, bert_tokenizer, jp_tokenizer, args) if args.mlm else (batch, batch)
...
# parser.add_argument("--model_type", default="bert", type=str,
    ...
    assert all([param.requires_grad for param in model.bert.encoder.layer[-1].parameters()])
